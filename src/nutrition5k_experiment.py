...
from comp90086 import Nutrition5KDataset

# Import your model class
from nutrition5k_inceptionv3_model import build_nutrition5k_model

# Import your trainer class and utilities
from comp90086 import Trainer, get_warmup_cosine_scheduler

def train_nutrition5k_model(
    fusion='middle',
    data_root='../Nutrition5K/train',
    train_csv='../Nutrition5K/train_split.csv',
    val_csv='../Nutrition5K/val_split.csv',
    ...
    """
    Train the Nutrition5k model with InceptionV3 architecture
    
    Args:
        fusion: Fusion type ('early', 'middle', or 'late')
        data_root: Root directory for the dataset
        train_csv/val_csv: CSV files for train/val splits
        output_dir: Directory to save results
        batch_size/num_epochs/etc: Training hyperparameters
    ...
    print("="*60)
    print(f"TRAINING: Nutrition5k Paper InceptionV3 + {fusion.capitalize()} Fusion")
    print("="*60)
    ...
    # Build the Nutrition5k InceptionV3 model
    ...
    exp_name = f"inceptionv3_{fusion}_fusion_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
if __name__ == "__main__":
    ...
    middle_fusion_results = train_nutrition5k_model(fusion='middle')
    
    # Train with early fusion
    early_fusion_results = train_nutrition5k_model(fusion='early')
    
    # Train with late fusion
    late_fusion_results = train_nutrition5k_model(fusion='late')
